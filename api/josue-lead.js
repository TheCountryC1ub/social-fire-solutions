// Vercel serverless function — receives the "Work with Josue" form from
// /josue-sanchez and pushes it into GoHighLevel (v2 / LeadConnector).
// Same pattern + env vars as /api/lead and /api/madeline-lead:
...
function noteBody(d) {
  return [
    "WORK WITH JOSUE — JOSUE SANCHEZ (socialfire.solutions/josue-sanchez)",
    "",
    `Name: ${(d.name || "").toString().trim() || "—"}`,
    `Email: ${(d.email || "").toString().trim() || "—"}`,
    `Phone: ${(d.phone || "").toString().trim() || "—"}`,
    `They need: ${(d.interest || "").toString().trim() || "—"}`,
    "",
    "Their note:",
...
module.exports = async (req, res) => {
  ...
  const tags = ["Josue Sanchez Lead"];
  if (d.interest) tags.push(`JS interest: ${d.interest}`);
  ...
  try {
    ...
    const upsertRes = await fetch(`${GHL_BASE}/contacts/upsert`, {
      ...
      body: JSON.stringify({
        ...
        source: "Josue Sanchez Page (Website)",
      }),
    });
