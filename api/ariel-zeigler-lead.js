// Vercel serverless function — receives the Book a Call form from
// /ariel-zeigler and pushes it into GoHighLevel (v2 / LeadConnector).
// Same pattern + env vars as /api/lead and /api/free-website:
...
function noteBody(d) {
  return [
    "BOOK A CALL — ARIEL ZEIGLER (socialfire.solutions/ariel-zeigler)",
    "",
    `Name: ${(d.name || "").toString().trim() || "—"}`,
    `Email: ${(d.email || "").toString().trim() || "—"}`,
    `Phone: ${(d.phone || "").toString().trim() || "—"}`,
    `Interested in: ${(d.interest || "").toString().trim() || "—"}`,
    "",
    "What's going on:",
...
module.exports = async (req, res) => {
  ...
  const tags = ["Ariel Zeigler Lead"];
  if (d.interest) tags.push(`AZ interest: ${d.interest}`);
  ...
  try {
    ...
    const upsertRes = await fetch(`${GHL_BASE}/contacts/upsert`, {
      ...
      body: JSON.stringify({
        ...
        source: "Ariel Zeigler Page (Website)",
      }),
    });
