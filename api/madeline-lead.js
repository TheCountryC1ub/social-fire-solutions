// Vercel serverless function — receives the Reach Out form from
// /madeline-taylor-reams and pushes it into GoHighLevel (v2 / LeadConnector).
// Same pattern + env vars as /api/lead and /api/ariel-zeigler-lead:
...
function noteBody(d) {
  return [
    "REACH OUT — MADELINE TAYLOR REAMS (socialfire.solutions/madeline-taylor-reams)",
    "",
...
module.exports = async (req, res) => {
  ...
  const tags = ["Madeline Reams Lead"];
  if (d.interest) tags.push(`MR interest: ${d.interest}`);
  ...
  try {
    ...
    const upsertRes = await fetch(`${GHL_BASE}/contacts/upsert`, {
      ...
      body: JSON.stringify({
        ...
        source: "Madeline Taylor Reams Page (Website)",
      }),
    });
    ...
    if (contactId) {
      ...
      // 3) Attach the request as a note (best-effort — never fails the lead)
      try {
