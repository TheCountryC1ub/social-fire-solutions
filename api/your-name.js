// Vercel serverless function — receives the Personal Website ("Your Name")
// survey and pushes it into GoHighLevel (v2 / LeadConnector). Same pattern as
// /api/free-website.
//
// Uses the SAME environment variables as /api/free-website (already set in
// Vercel):
...
const FIELD_ORDER = [
  ["hassite", "What their name shows today"],
  ["purpose", "Site is for"],
...
function noteBody(d) {
  const lines = ["PERSONAL WEBSITE SURVEY (Your Name funnel)", ""];
    ...
    data: [
      {
        ...
        event_source_url:
          req.headers.referer || "https://socialfire.solutions/your-name",
...
module.exports = async (req, res) => {
  ...
  const { firstName, lastName } = splitName(d.name);
  const tags = ["Personal Website Survey"];
  if (d.purpose) tags.push(`For: ${d.purpose}`);
  ...
  try {
    ...
    const upsertRes = await fetch(`${GHL_BASE}/contacts/upsert`, {
      ...
      body: JSON.stringify({
        ...
        source: "Your Name Funnel (Website)",
      }),
    });
